...
public class GatherRouteDB : Configuration.Node {
    ...
    public enum Movement {
        ...
        MountFly = 1,
        [LocalizedDescription("Ground Mount", "地面坐骑")]
        MountNoFly = 2,
    }
    ...
    public enum InteractionType {
        ...
        Standard = 1,
        [LocalizedDescription("Start Route", "开始路线")]
        StartRoute = 9,
        [LocalizedDescription("Scan for Gathering Nodes", "扫描采集点")]
        NodeScan = 12,
    }
        ...
        try {
            ...
            if (import.Waypoints.Any(x => (x.Pathfind || x.Interaction == InteractionType.NodeScan) && !Service.Navmesh.IsEnabled))
                Service.ChatGui.Print(Loc.Format(
                    "[{0}] Imported route uses pathfinding, but vnavmesh is not installed. It's located on the same repo as {0} ({1}).",
                    "[{0}] 导入的路线使用了路径规划，但当前未安装 vnavmesh。它和 {0} 位于同一个仓库（{1}）。",
                    Plugin.Name,
                    Plugin.Repo));
