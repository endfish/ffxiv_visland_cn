    ...
    public WorkshopSolver.Recs BuildRecs(int season) {
        if (!_seasons.TryGetValue(season, out var seasonRec))
            throw new Exception(Loc.Format(
                "No archived recommendations for season {0} (have {1}-{2})",
                "没有第 {0} 赛季的历史推荐（当前包含 {1}–{2}）",
                season,
                RangeStart,
                RangeEnd));
    ...
    private WorkshopSolver.WorkshopRec BuildWorkshop(List<uint> crafts) {
        ...
        foreach (var id in crafts) {
            if (MJICraftworksObject.GetRow(id) is not { } row)
                throw new Exception(Loc.Format("Unknown craftworks id {0}", "未知工艺 ID：{0}", id));
            rec.Add(hour, id);
            hour += row.CraftingTime;
        }
        if (hour > 24)
            throw new Exception(Loc.Format("Workshop schedule exceeds 24h ({0})", "工坊排班超过 24 小时（{0}）", hour));
        return rec;
    }
    ...
    private void LoadEmbedded() {
        ...
        using var stream = asm.GetManifestResourceStream("visland.Workshop.Data.workshop-seasons.json")
            ?? throw new Exception(Loc.Tr(
                "Embedded resource visland.Workshop.Data.workshop-seasons.json not found",
                "未找到内嵌资源 visland.Workshop.Data.workshop-seasons.json"));
        using var reader = new StreamReader(stream);
        var root = JObject.Parse(reader.ReadToEnd());

        var range = root["range"]?.ToObject<int[]>()
            ?? throw new Exception(Loc.Tr("workshop-seasons.json: missing range", "workshop-seasons.json：缺少 range"));
        RangeStart = range[0];
        RangeEnd = range[1];
    ...
    private static List<uint>? ParseCraftIds(JArray? arr, int season, int cycle, string workshop) {
        ...
        foreach (var token in arr) {
            if (token.Type != JTokenType.Integer)
                throw new Exception(Loc.Format(
                    "Season {0} C{1} {2}: expected craft ids, got {3}",
                    "第 {0} 赛季 C{1} {2}：应为工艺 ID，实际为 {3}",
                    season,
                    cycle,
                    workshop,
                    token.Type));
